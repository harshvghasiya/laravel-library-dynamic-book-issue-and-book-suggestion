...
<div class="col-lg-3 col-lg-offset-1 col-md-4 col-sm-12 col-xs-12">
  <aside aria-label="sidebar" class="sidebar sidebar-right">
    <div class="widget">
      <form class="w-search" action="{{route('front.blog.search_post')}}" method="get">
        @if(Request::get('search') !="")
        <input class="email search input-standard-grey" name="search"  placeholder="Search" type="search" value="{{Request::get('search')}}">
        @else
    ...
    @if(isset($categoryCount) && !$categoryCount->isEmpty())
    <div class="widget w-post-category">
      ...
      <div class="post-category-wrap">
        @foreach($categoryCount as $key=>$v)
          <div class="category-post-item">
            <span class="post-count">{{$v->multiple_category_status_count}}</span>
            <a href="{{route('front.category.category_single',$v->slug)}}" class="category-title">{{$v->name}}
    ...
    @endif
    @if(isset($recentBlog) && !$recentBlog->isEmpty())
      ...
      <div class="widget w-tags">
        ...
        <div class="tags-wrap">
          @foreach($tag as $key=>$v)
           <a href="{{route('front.tag.detail',$v->slug)}}" class="w-tags-item">{{$v->title}}</a>
          @endforeach
        </div>
      </div>
    @endif
