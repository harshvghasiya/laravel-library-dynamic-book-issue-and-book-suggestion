...
<ul class="page-breadcrumb breadcrumb">
  ...
  <li class="active">
   <?php echo e(trans('lang_data.menu_manage')); ?>
        ...
        <div class="portlet light">
          ...
          <div class="portlet-body">
            <div class="row">
              ...
              <?php $__currentLoopData = $headerSectionLink; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <li>
                <div class="row">
                  <div class="col-md-4">
                    <h5 style="font-size: 15px;"><?php echo e($value->name); ?></h5>
                  </div> 
                  <div class="col-md-6"> 
                   <button type="buttton" id="add_to_menu_btn" data-id="<?php echo e(Crypt::encrypt($value->id)); ?>" class="btn blue">Add Menu</button>
                  </div>
                </div>
              </li><hr>
...
<script type="text/javascript">
  ...
  $(document).on('click', '.save_menu', function(event) {
    ...
    var op=$(this);
    $.ajax({
      url: '<?php echo e(route('admin.menu.sorting')); ?>',
      type: 'POST',
  ...
  $(document).on('click', '.remove_menu', function(event) {
    ...
    var op=$(this);
    $.ajax({
      url: '<?php echo e(route('admin.menu.remove_menu')); ?>',
      type: 'POST',
  ...
  $(document).on('submit', 'form.menuitem_edit', function(event) {
    ...
    $.ajax({
      ...
      url: '<?php echo e(route('admin.menu.menuitem_edit')); ?>',
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      type: 'POST',
  ...
  $(document).on('submit', 'form.addupdatemenuform', function(event) {
    ...
    $.ajax({
      ...
      url: '<?php echo e(route('admin.menu.menuitem_store')); ?>',
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      type: 'POST',
  ...
  function MenuStrucutre() {
    $.ajax({
      url: '<?php echo e(route('admin.menu.menu_strucutre_data')); ?>',
      type: 'POST',
      success:function(response) {
