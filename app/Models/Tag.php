...
{
    protected $table="tags"; // To define dabase table
     ...
     * @author Chirag G.
     */
    public function getTagListData($request){
     ...
     * @author Chirag G.
     */
    public function saveTag($r,$id=NULL){
    ...
    public function tagAjax($request){

       if($request->ajax()){

            $tagDetail = \App\Models\Tag::where('slug',$request->tag)->where('status',\App\Models\Tag::STATUS_ACTIVE)->first();
            $blogList  = \App\Models\Blog::select('id','slug','name','image',
                                    'short_description','total_visitor','status','created_at','created_by')
                                        ->has('multipleBlogCategory')
                                          ->with(['multipleBlogCategory','multipleBlogCategory.category','multipleTag','multipleTag.tag'])
                                          ...
                                          ->whereHas('multipleTag',function($rc) use($request){

                                            $rc->whereHas('tag',function($rcd) use($request){

                                                $rcd->where('slug',$request->tag);
        ...
        return view('front.blog.pagination_all_blog',compact("blogList","tagDetail"));
         }   
