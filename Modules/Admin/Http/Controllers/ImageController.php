...
{ 	
	...
	public function delete(Request $request,$id)
	{
			...
			$productImage = \App\Models\ProductImage::where('id',$id)->firstOrFail();
        
            if ($productImage->name !=null) {

                if(file_exists(PRODUCT_IMAGE_UPLOAD_PATH().$productImage->name))
                {
                    \File::delete(PRODUCT_IMAGE_UPLOAD_PATH().$productImage->name);
                }

                if(file_exists(PRODUCT_IMAGE_UPLOAD_PATH().'thumbnail_'.$productImage->name))
                {
                    \File::delete(PRODUCT_IMAGE_UPLOAD_PATH().'thumbnail_'.$productImage->name);
                }

                $productImage->delete();
            }

            return response()->json(['success'=>1,'msg'=>"Successfully deleted."]);
	...
	public function sort(Request $request)
	{
        ...
        foreach ($images as $image) {
        	$image->update([ 'order_id' => $order[$image->id] ]);
        }
