...
<ul class="page-breadcrumb breadcrumb">
  ...
  <li class="active">
     {{trans('lang_data.country_list_label')}}
  </li>
</ul>
<div class="row">
  <div class="col-md-12">
    ...
    <div class="portlet box grey-cascade">
      <div class="portlet-title">
        <div class="caption">
          <i class="fa fa-globe"></i>{{trans('lang_data.location_management_keyword')}}
        </div>
        <div class="tools">
      ...
      <div class="portlet-body">
        <div class="table-toolbar">
          <div class="row">
            <div class="col-md-6">
              <div class="btn-group">
                <a href="{{route('admin.country.create')}}" id="sample_editable_1_new" class="btn green">
                {{trans('lang_data.add_new_label')}} <i class="fa fa-plus"></i>
                </a>
              </div>
              @if(env('ACCESS_TO_DELETE') == true)
              <div class="btn-group">
                <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.country.delete_all')}}')">{{trans('lang_data.delete_all_label')}}
                </a>
              </div>
              @endif
              <div class="btn-group">
                <a class="btn btn-warning" onclick="statusAll('table_form','{{route('admin.country.status_all')}}')">{{trans('lang_data.status_all_label')}}
                </a>
              </div>
            </div>
          </div>
...
<script>
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.country.any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.country.single_status_change')}}",
