...
<ul class="page-breadcrumb breadcrumb">
  ...
  <li class="active">
     {{trans('lang_data.portfolio_listing')}}
  </li>
</ul>
<div class="row">
  <div class="col-md-12">
    ...
    <div class="portlet box grey-cascade">
      <div class="portlet-title">
        <div class="caption">
          <i class="fa fa-globe"></i>{{trans('lang_data.managed_portfolio_label')}}
        </div>
        <div class="tools">
      ...
      <div class="portlet-body">
        <div class="table-toolbar">
          <div class="row">
            <div class="col-md-6">
              <div class="btn-group">
                <a href="{{route('admin.portfolio.create')}}" id="sample_editable_1_new" class="btn green">
                Add New <i class="fa fa-plus"></i>
                </a>
              </div>
              @if(env('ACCESS_TO_DELETE') == true)
                <div class="btn-group">
                  <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.portfolio.delete_all')}}')">{{trans('lang_data.delete_all_label')}}
                  </a>
                </div>
              @endif
              <div class="btn-group">
                <a class="btn btn-warning" onclick="statusAll('table_form','{{route('admin.portfolio.status_all')}}')">{{trans('lang_data.status_all_label')}}
                </a>
              </div>
            </div>
          </div>
        ...
        <table class="table table-striped table-bordered table-hover" id="users-table">
          <thead>
            ...
            <th>{{trans('lang_data.portfolio')}} {{trans('lang_data.name_label')}}</th>
            <th>{{trans('lang_data.total_category')}}</th>
...
<script>
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.portfolio.any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.portfolio.single_status_change')}}",
