...
<ul class="page-breadcrumb breadcrumb">
  ...
  <li class="active">
     {{trans('lang_data.category_list_label')}}
  </li>
</ul>
<div class="row">
  <div class="col-md-12">
            ...
            <!-- form-inline -->
              <form method="POST" id="search-form" class="" role="form">
                ...
                <div class="row">
                  ...
                  <div class="col-md-3">
                    <div class="form-group">
                        <label for="name">{{trans('lang_data.show_on_header_label')}}</label>
                        <select name="show_on_header" class="form-control">
                          <option value="">{{trans('lang_data.select_header_status_label')}}</option>
                          <option value="1">{{trans('lang_data.yes_label')}}</option>
                          <option value="0">{{trans('lang_data.no_label')}}</option>
                        </select>
                    </div>
                  </div>
                  <div class="col-md-3">
                    <div class="form-group">
                        <label for="name">{{trans('lang_data.show_on_footer_label')}}</label>
                        <select name="show_on_footer" class="form-control">
                          <option value="">{{trans('lang_data.select_footer_status_label')}}</option>
                          <option value="1">{{trans('lang_data.yes_label')}}</option>
                          <option value="0">{{trans('lang_data.no_label')}}</option>
                        </select>
                    </div>
    ...
    <div class="portlet box grey-cascade">
      <div class="portlet-title">
        <div class="caption">
          <i class="fa fa-globe"></i>{{trans('lang_data.managed_category_label')}}
        </div>
        <div class="tools">
      ...
      <div class="portlet-body">
        <div class="table-toolbar">
          <div class="row">
            <div class="col-md-6">
              <div class="btn-group">
                <a href="{{route('admin.category.create')}}" id="sample_editable_1_new" class="btn green">
                {{trans('lang_data.add_new_label')}} <i class="fa fa-plus"></i>
                </a>
              </div>
              @if(env('ACCESS_TO_DELETE') == true)
              <div class="btn-group">
                <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.category.delete_all')}}')">{{trans('lang_data.delete_all_label')}}
                </a>
              </div>
              @endif
              <div class="btn-group">
                <a class="btn btn-warning" onclick="statusAll('table_form','{{route('admin.category.status_all')}}')">{{trans('lang_data.status_all_label')}}
                </a>
              </div>
            </div>
          </div>
...
<script>
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.category.any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.category.single_status_change')}}",
