...
<ul class="page-breadcrumb breadcrumb">
  ...
  <li class="active">
     {{trans('lang_data.email_template_list_label')}}
  </li>
</ul>
<div class="row">
  <div class="col-md-12">
              ...
              <form method="POST" id="search-form" class="form-inline" role="form">
                {{ csrf_field() }}
                  <div class="form-group">
                      <label for="name">{{trans('lang_data.title_label')}}</label>
                      <input type="text" class="form-control" name="title" id="name" placeholder="{{trans('lang_data.search_title_label')}}">
                  </div> 
                  <div class="form-group">
                      <label for="name">{{trans('lang_data.subject_label')}}</label>
                      <input type="text" class="form-control" name="subject" id="name" placeholder="{{trans('lang_data.search_subject_label')}}">
                  </div>
                 <div class="form-group">
    ...
    <div class="portlet box grey-cascade">
      <div class="portlet-title">
        <div class="caption">
          <i class="fa fa-globe"></i>{{trans('lang_data.managed_e_template_label')}}
        </div>
        <div class="tools">
      ...
      <div class="portlet-body">
        <div class="table-toolbar">
          <div class="row">
            <div class="col-md-6">
              <div class="btn-group">
                <a href="{{route('admin.email-template.create')}}" id="sample_editable_1_new" class="btn green">
                Add New <i class="fa fa-plus"></i>
                </a>
              </div>
              @if(env('ACCESS_TO_DELETE') == true)
              <div class="btn-group">
                <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.email-template.delete_all')}}')">{{trans('lang_data.delete_all_label')}}
                </a>
              </div>
              @endif
              <div class="btn-group">
                <a class="btn btn-warning" onclick="statusAll('table_form','{{route('admin.email-template.status_all')}}')">{{trans('lang_data.status_all_label')}}
                </a>
              </div>
            </div>
          </div>
          ...
          <thead>
            <tr>
             <th class="table-checkbox">
              ...
              <th>{{trans('lang_data.title_label')}}</th>
              <th>{{trans('lang_data.from_label')}}</th>
...
<script>
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.email-template.any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.email-template.single_status_change')}}",
