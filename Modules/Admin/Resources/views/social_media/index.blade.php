...
<ul class="page-breadcrumb breadcrumb">
  ...
  <li class="active">
     {{trans('lang_data.social_media_list')}}
  </li>
</ul>
<div class="row">
  <div class="col-md-12">
              ...
              <form method="POST" id="search-form" class="form-inline" role="form">
                {{ csrf_field() }}
                  <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="title" id="title" placeholder="{{trans('lang_data.search_name_placeholder')}}">
                  </div>
                  <div class="form-group">
                      <label for="name">{{trans('lang_data.status_label')}}</label>
                      <select name="status" class="form-control">
                        <option value="">{{trans('lang_data.select_label')}} {{trans('lang_data.status_label')}}</option>
    ...
    <div class="portlet box grey-cascade">
      <div class="portlet-title">
        <div class="caption">
          <i class="fa fa-globe"></i>{{trans('lang_data.managed_social_label')}}
        </div>
        <div class="tools">
      ...
      <div class="portlet-body">
        <div class="table-toolbar">
          <div class="row">
            <div class="col-md-6">
              <div class="btn-group">
                <a href="{{route('admin.social_medias.create')}}" id="sample_editable_1_new" class="btn green">
                Add New <i class="fa fa-plus"></i>
                </a>
              </div>
              @if(env('ACCESS_TO_DELETE') == true)
              <div class="btn-group">
                <a class="btn btn-danger" onclick="deleteAll('table_form','{{route('admin.social_medias.delete_all')}}')">{{trans('lang_data.delete_all_label')}}
                </a>
              </div>
              @endif
              <div class="btn-group">
                <a class="btn btn-warning" onclick="statusAll('table_form','{{route('admin.social_medias.status_all')}}')">{{trans('lang_data.status_all_label')}}
                </a>
              </div>
            </div>
          </div>
...
<script>
  var oTable = $('#users-table').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('admin.social_medias.any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click","#active_inactive",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('admin.social_medias.single_status_change')}}",
